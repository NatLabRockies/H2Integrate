...
def set_environment_variable(env_var: str):
    global generic_api_key
    generic_api_key = env_var
...
def load_file_with_env_variables(
    fpath,
    env_variable_name: str,
    legacy_env_variable_name: str | None = None,
    set_func=set_environment_variable,
):
    ...
    var = env_variable_name
    line_w_var = [line for line in lines if var in line]
    if len(line_w_var) == 0 and legacy_env_variable_name is not None:
        line_w_var = [line for line in lines if legacy_env_variable_name in line]
        if len(line_w_var) > 0:
            warnings.warn(
                _DEPRECATION_MSG.format(old=legacy_env_variable_name, new=env_variable_name),
                FutureWarning,
                stacklevel=2,
            )
            var = legacy_env_variable_name  # use old name for parsing
    if len(line_w_var) != 1:
        raise ValueError(
    ...
    valid_keynames = (
        (env_variable_name, legacy_env_variable_name)
        if legacy_env_variable_name is not None
        else (env_variable_name)
    )
    ...
    if var in valid_keynames:
        set_func(val)

    return


def set_api_key_using_dot_env(variable_name: str, alternative_name: str, path=None):
...
def set_api_key_dot_env(path=None):
...
def get_nlr_developer_api_key(env_path=None):
    ...
    2) check if the key has already been set as a global variable from
        running `set_api_key_dot_env()`. If not set, proceed to Step 3.
    3) Attempt to set the key by calling `set_api_key_dot_env()`.
    ...
    if len(developer_nlr_gov_key) == 0:
        ...
        set_api_key_dot_env(path=env_path)
...
def get_nlr_developer_api_email(env_path=None):
    ...
    2) check if the email has already been set as a global variable from running
        `set_api_key_dot_env()`. If not set, proceed to Step 3.
    3) Attempt to set the email by calling `set_api_key_dot_env()`.
