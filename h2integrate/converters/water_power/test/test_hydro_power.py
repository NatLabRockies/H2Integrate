    ...
    plant_config = {
        ...
        "site": {
            "resources": {
                "river_resource": {
                    "resource_parameters": {
                        "filename": EXAMPLE_DIR / "07_run_of_river_plant" / "river_data.csv"
                    }
                }
            }
        },
...
def test_hydro_power_performance_outputs(tech_config, plant_config, subtests):
    ...
    river_resource = RiverResource(
        ...
        resource_config=plant_config["site"]["resources"]["river_resource"]["resource_parameters"],
        driver_config={},
    )
    prob.model.add_subsystem("river_resource", river_resource, promotes=["*"])

    # START HERE
    comp = RunOfRiverHydroPerformanceModel(
        plant_config=plant_config,
