...
def _make_plant_config(
    ...
    config = {
        ...
        "system_level_control": {
            ...
            "control_parameters": {"commodity_sell_price": sell_price},
        },
    }
...
def _make_slc_config(plant_config):
    """Build a minimal slc_config dict from a plant_config for controller tests."""
    slc = plant_config["system_level_control"]
    ...
    for t in all_techs:
        tech_graph.add_node(t)
    tech_graph.add_node(slc["demand_tech"])
    for t in all_techs:
        tech_graph.add_edge(t, slc["demand_tech"], commodity=slc["demand_commodity"])
    for conn in plant_config.get("technology_interconnections", []):
        if len(conn) >= 4:
            tech_graph.add_edge(conn[0], conn[1], commodity=conn[2])
        else:
            tech_graph.add_edge(conn[0], conn[1])

    # Build tech_control_classifiers
    classifiers = {}
    for t in curtailable:
        classifiers[t] = "curtailable"
    ...
    return {
        ...
        "demand_commodity_rate_units": slc.get("demand_commodity_rate_units"),
        "demand_tech": slc["demand_tech"],
        "tech_to_commodity": slc["tech_to_commodity"],
        "storage_techs_to_control": {},
        "technology_graph": tech_graph,
        "tech_control_classifiers": classifiers,
    }
...
def _build_problem(slc_cls, plant_config, tech_config=None):
    ...
    slc_cfg = plant_config["system_level_control"]
    demand_name = f"slc.{slc_cfg['demand_commodity']}_demand"
    prob.set_val(demand_name, slc_cfg["demand_profile"])
    ...
    def test_buy_price_scalar(self):
        ...
        prob.model.add_subsystem(
            ...
            ProfitMaximizationControl(
                ...
                slc_config=_make_slc_config(pc),
            ),
        )
        prob.setup()
