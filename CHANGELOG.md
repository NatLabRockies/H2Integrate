...
- Add heterogeneous-commodity system-level control that translates demand for one commodity into upstream set-points across converters, deriving per-technology conversion ratios automatically rather than reading them from the config. [PR TBD](https://github.com/NatLabRockies/H2Integrate/pull/TBD)
  ...
  - Add the `35_system_level_control/heterogeneous_commodity` example, which serves an ammonia demand from a wind, solar, battery, electrolyzer, hydrogen-storage, and ammonia synthesis loop chain to demonstrate demand propagating from ammonia to hydrogen to electricity. The example combines the wind and solar generation into a single stream that charges the battery and feeds the downstream electricity bus. It uses profit-maximizing control so that wind and solar always run and the battery charges on surplus generation and discharges to cover short deficits. There is no firm grid backup, so the ammonia demand is only met when wind, solar, and the battery can supply enough electricity, and the example generates dispatch and dynamic conversion-ratio figures. [PR TBD](https://github.com/NatLabRockies/H2Integrate/pull/TBD)
