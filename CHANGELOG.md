...
- Exposed `n_timesteps`, `dt`, `plant_life`, and `fraction_of_year_simulated` as attributes on `CostModelBaseClass` (matching `PerformanceModelBaseClass`) and updated all cost and performance model subclasses across `h2integrate/` to use these attributes instead of re-reading them from `plant_config`, removing redundant boilerplate from individual components. Resolves [#708](https://github.com/NatLabRockies/H2Integrate/issues/708). [PR 783](https://github.com/NatLabRockies/H2Integrate/pull/783)
