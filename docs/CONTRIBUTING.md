...
* Code formatting is enforced using pre-commit hooks. The pre-commit package is included in the developer install of the repository. The pre-commit hooks can be installed by running
```bash
pre-commit install
```
